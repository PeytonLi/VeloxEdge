...
interface OriginFetchResult extends CachedAsset {
  originMs: number;
}
...
function ttlExpires(seconds: number): number {
  return wallNow() + Math.max(1, seconds) * 1000;
}
function computeMicros(startedAt: number): number {
  return Math.max(0, Math.round((nowMs() - startedAt) * 1000));
}
function measuredMs(startedAt: number): number {
  return Math.max(0, Math.round((nowMs() - startedAt) * 100) / 100);
}
function ttlGet<T>(map: Map<string, TtlItem<T>>, key: string): T | null {
  const item = map.get(key);
  if (!item) return null;
  if (item.expiresAt <= wallNow()) {
    map.delete(key);
    return null;
  }
  return item.value;
...
function ttlSet<T>(
  map: Map<string, TtlItem<T>>,
  key: string,
  value: T,
  seconds: number,
): void {
  map.set(key, { value, expiresAt: ttlExpires(seconds) });
}
...
function persistEngine(sessionId: string, engine: LinUCBEngine): void {
  ...
  try {
    ttlSet(states, sessionId, engine.serialize(), VELOX_STATE_TTL_SECONDS);
  } catch {
...
async function fetchOriginAsset(key: string): Promise<OriginFetchResult> {
  ...
  try {
    ...
    if (!response.ok)
      throw new Error("origin returned " + String(response.status));
    return {
      key,
      bytes,
      contentType:
    ...
    await new Promise((resolve) =>
      setTimeout(resolve, descriptor.coldOriginMs),
    );
    return {
      key,
...
async function prefetchAsset(
  key: string,
): Promise<{ originMs: number; cacheWritten: boolean }> {
...
async function resolveAsset(
  key: string,
): Promise<{ cacheHit: boolean; latencyMs: number; coldOriginMs: number }> {
  const startedAt = nowMs();
  ...
  if (cached) {
    ...
    return {
      cacheHit: true,
      latencyMs: measuredMs(startedAt),
  ...
  return {
    cacheHit: false,
    latencyMs: measuredMs(startedAt),
    coldOriginMs: fetched.coldOriginMs,
  };
}

function predictionStep(requestStep: number | undefined): number {
  return typeof requestStep === "number" && Number.isFinite(requestStep)
    ? requestStep
    : wallNow();
}
...
export function createEdgeKvEmulator(): EdgeKvEmulator {
  return {
    async predict(request) {
      ...
      const predictedKey =
        prediction.action === "NO_OP"
          ? ""
          : deriveAssetKey(request.contextVector, prediction.action);
      const step = predictionStep(request.step);

      // NO_OP means "don't pre-fetch anything" — skip the fetch entirely
      const prefetch =
        prediction.action === "NO_OP"
          ? { executed: false, key: "", originMs: 0, cacheWritten: false }
          : await (async () => {
              const result = await prefetchAsset(predictedKey);
              return {
                executed: true,
                key: predictedKey,
    ...
    async resolve(request) {
      ...
      const prediction = pending
        ...
        : {
            action: engine.predictNextAction(request.contextVector).action,
            contextVector: request.contextVector,
          };
      const assetResult = await resolveAsset(request.requestedKey);
      const reward = rewardFromLatency(
        assetResult.latencyMs,
        EDGE_HIT_MS,
        assetResult.coldOriginMs,
      );
