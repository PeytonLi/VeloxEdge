...
const STEPPER = [
  {
    time: "001µs",
    label: "Prompt delta hashed",
    description: "deterministic keyword → latent vector",
  },
  {
    time: "006µs",
    label: "A⁻¹ variance read",
    description: "per-arm covariance confidence",
  },
  {
    time: "013µs",
    label: "LinUCB argmax",
    description: "reward + α·exploration bonus",
  },
  {
    time: "021µs",
    label: "Edge prefetch issued",
    description: "asset staged near the agent",
  },
];
  ...
  return (
    <article className="console-card">
      <div className="console-topline">
        ...
        <span
          className={`engine-badge ${ready ? "engine-live" : "engine-demo"}`}
        >
        ...
        <div className="console-actions">
          {runState === "running" ? (
            <>
              ...
              <button className="ghost-action" type="button" onClick={onStop}>
                ⏹ Stop
              </button>
            </>
          ) : runState === "paused" ? (
      ...
      <div className="journey-context">
        ...
        <span>
          {selectedJourney.steps.length || DEMO_JOURNEYS[0].steps.length} latent
          turns · current: {activeStepLabel}
        </span>
      </div>
      ...
      <div className="micro-stepper" aria-label="Microsecond execution stepper">
        {STEPPER.map((step, index) => (
          <div
            className={`micro-step ${busy || index < 3 ? "micro-step-active" : ""}`}
            key={step.label}
          >
            <span>{step.time}</span>
