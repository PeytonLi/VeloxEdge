...
import { LinUCBEngine, deriveAssetKey, rewardFromLatency } from "@veloxedge/bandit-engine";
import type { EdgePredictResponse, EdgeResolveResponse, EngineSnapshot, PredictionResult } from "@veloxedge/bandit-engine";
...
import { COLD_FETCH_MS, NaiveStringCache, accumulateStats, embed, emptyStats, journeys } from "@/lib/simulation";
...
interface Scenario { contextVector: number[]; label: string; naiveKey: string; }
interface StepOutcome { prediction: PredictionResult; latency: LatencyResult; snapshot: EngineSnapshot | null; rttMs: number | null; computeMicros: number | null; source: VeloxEngineMode; requestedKey: string; }
interface LocalCachedAsset { coldOriginMs: number; expiresAt: number; }
...
async function resolveScenario(input: string | number[], dimensions: number, turn: number): Promise<Scenario> {
  if (Array.isArray(input)) {
    const contextVector = resizeVector(input, dimensions);
    const step = nearestJourneyStep(contextVector, dimensions);
    return { contextVector, label: step.label, naiveKey: step.label + " :: latent-turn=" + String(turn) };
  }

  const trimmed = input.trim();
  const label = trimmed.length > 0 ? trimmed : "Untitled live prompt";
  const adapter = createEmbeddingAdapter({ provider: "deterministic" });
  let contextVector: number[];
  try { contextVector = await adapter.embed(label, dimensions); }
  catch { contextVector = embed(label, journeys, dimensions); }
  return { contextVector: resizeVector(contextVector, dimensions), label, naiveKey: label.toLowerCase() + " :: json_args_nonce=" + String(turn) };
}
...
function predictionFromResolve(response: EdgeResolveResponse, selectedAction: string): PredictionResult { return { action: selectedAction, ucbBreakdown: response.ucbBreakdown }; }

function latencyFromMeasured(cacheHit: boolean, latencyMs: number, reward: number, coldOriginMs: number): LatencyResult {
  return { cacheHit, latencyMs: roundMs(latencyMs), savedMs: cacheHit ? roundMs(Math.max(0, coldOriginMs - latencyMs)) : 0, reward };
}

async function measuredLocalResolve(key: string, cache: Map<string, LocalCachedAsset>): Promise<{ cacheHit: boolean; latencyMs: number; coldOriginMs: number; reward: number }> {
  const cached = cache.get(key);
  const startedAt = clockNow();
  if (cached && cached.expiresAt > Date.now()) {
    await delay(EDGE_HIT_MS);
    const latencyMs = roundMs(clockNow() - startedAt);
    return { cacheHit: true, latencyMs, coldOriginMs: cached.coldOriginMs, reward: rewardFromLatency(latencyMs, EDGE_HIT_MS, cached.coldOriginMs) };
  }
  cache.delete(key);
  const descriptor = descriptorForAsset(key);
  await delay(descriptor.coldOriginMs);
  const latencyMs = roundMs(clockNow() - startedAt);
  cache.set(key, { coldOriginMs: descriptor.coldOriginMs, expiresAt: Date.now() + LOCAL_ASSET_TTL_MS });
  return { cacheHit: false, latencyMs, coldOriginMs: descriptor.coldOriginMs, reward: rewardFromLatency(latencyMs, EDGE_HIT_MS, descriptor.coldOriginMs) };
}

function buildEvents(baseTimestamp: number, scenario: Scenario, prediction: PredictionResult, result: LatencyResult, naiveAction: string | null, mode: VeloxEngineMode, rttMs: number | null, computeMicros: number | null, requestedKey: string): InterceptorEvent[] {
  ...
  const runtime = mode === "edge" ? "LIVE EDGE rtt=" + (rttMs?.toFixed(1) ?? "n/a") + "ms compute=" + (computeMicros ?? 0) + "µs" : "LOCAL measured cache";
  const outcome = result.cacheHit ? "Measured cache HIT for " + requestedKey + " in " + result.latencyMs + "ms" : "Measured cache MISS for " + requestedKey + " in " + result.latencyMs + "ms";
...
export function useVeloxEngine(_dimensions?: number, _alpha?: number): VeloxEngineState & VeloxEngineActions {
  ...
  const runLocalStep = useCallback(async (scenario: Scenario): Promise<StepOutcome> => {
    ...
    const requestedKey = deriveAssetKey(scenario.contextVector, prediction.action);
    const measured = await measuredLocalResolve(requestedKey, localAssetCacheRef.current);
    const latency = latencyFromMeasured(measured.cacheHit, measured.latencyMs, measured.reward, measured.coldOriginMs);
    engine.updateWeights(prediction.action, scenario.contextVector, latency.reward);
    return { prediction, latency, snapshot: safeSnapshot(engine), rttMs: null, computeMicros: null, source: "local", requestedKey };
  }, [dimensions]);

  const runEdgeStep = useCallback(async (scenario: Scenario, stepNumber: number): Promise<StepOutcome> => {
    const requestConfig = { sessionId: sessionIdRef.current, dimensions, alpha: alphaRef.current, actions: ACTIONS };
    const predictResponse: EdgeClientResponse<EdgePredictResponse> = await edgeClient.predict({ ...requestConfig, contextVector: scenario.contextVector, step: stepNumber });
    const prediction = predictionFromEdge(predictResponse);
    const resolveResponse: EdgeClientResponse<EdgeResolveResponse> = await edgeClient.resolve({ sessionId: sessionIdRef.current, config: requestConfig, requestedKey: predictResponse.predictedKey, contextVector: scenario.contextVector, step: stepNumber });
    const latency = latencyFromMeasured(resolveResponse.cacheHit, resolveResponse.latencyMs, resolveResponse.reward, descriptorForAsset(resolveResponse.requestedKey).coldOriginMs);
    const resolvedPrediction = predictionFromResolve(resolveResponse, prediction.action);
    return {
      prediction: resolvedPrediction.ucbBreakdown.length > 0 ? resolvedPrediction : prediction,
      latency,
      snapshot: null,
      rttMs: predictResponse.rttMs + resolveResponse.rttMs,
      computeMicros: predictResponse.computeMicros + resolveResponse.computeMicros,
      source: "edge",
  ...
  const step = useCallback(async (input: string | number[]): Promise<void> => {
    ...
    try {
      ...
      setEvents((currentEvents) => currentEvents.concat(buildEvents(baseTimestamp, scenario, outcome.prediction, outcome.latency, naiveAction, outcome.source, outcome.rttMs, outcome.computeMicros, outcome.requestedKey)).slice(-MAX_EVENTS));
    } catch (error) {
