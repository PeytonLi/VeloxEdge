...
function updatePredictionFromEdge(
  ...
  const runtime =
    mode === "edge"
      ? "LIVE EDGE rtt=" +
        ...
        (computeMicros ?? 0) +
        "µs"
      : "LOCAL fallback compute in browser";
...
export function useVeloxEngine(
  ...
  const runEdgeStep = useCallback(
    async (scenario: Scenario): Promise<StepOutcome> => {
      ...
      const updateResponse: EdgeClientResponse<EdgeUpdateResponse> =
        await edgeClient.update({
          ...requestConfig,
          action: prediction.action,
          contextVector: scenario.contextVector,
          reward: latency.reward,
        });
      const updatePrediction = updatePredictionFromEdge(
        updateResponse,
        prediction.action,
      );
      return {
        prediction:
          updatePrediction.ucbBreakdown.length > 0
            ? updatePrediction
            : prediction,
        latency,
        snapshot: null,
        ...
        computeMicros:
          predictResponse.computeMicros + updateResponse.computeMicros,
        source: "edge",
      };
    },
    [dimensions],
